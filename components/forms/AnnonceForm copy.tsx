...
import { X, Upload, MapPin, Home, Users, Bed, Star, CheckCircle, Search, Plus, Building, Hotel, Castle, Building2, Mountain, Calendar, Navigation, Globe, Map, Loader2 } from "lucide-react";
import { Annonce, Amenity, User, categoriesAnnonce, UserLocation } from "@/types/interfaces";
...
const annonceSchema = z.object({
    ...
    // Category comme objet complet (doit correspondre à categoriesAnnonce)
    category: z.object({ id: z.string(), value: z.string(), label: z.string(), certifiedAt: z.string().optional(), }),
    // Category ID séparé
    ...
    // Provider ID (sera rempli automatiquement avec l'utilisateur connecté)
    providerId: z.string().optional(),
    // GPS Location (doit correspondre à UserLocation)
    ...
    // Review (doit correspondre à l'interface Review)
    review: z.object({
        id: z.string().optional(),
        author: z.string(),
...
// ================= ICON MAPPING POUR CATÉGORIES =================
const getCategoryIcon = (category: string) => {
    const icons: Record<string, React.ReactElement> = {
        Appartement: <Building className="w-5 h-5" />,
        Maison: <Home className="w-5 h-5" />,
        Villa: <Castle className="w-5 h-5" />,
        Studio: <Building2 className="w-5 h-5" />,
        Chambre: <Hotel className="w-5 h-5" />,
        Bureau: <Building className="w-5 h-5" />,
        Terrain: <Mountain className="w-5 h-5" />,
        Autre: <Plus className="w-5 h-5" />,
    };
    ...
    const handleFormSubmit: SubmitHandler<AnnonceFormValues> = useCallback(async (data) => {
        try {
            ...
            if (data.amenities && data.amenities.length > 0) {
                ...
                const amenityIds = data.amenities.map(amenity => amenity.id);
                formData.append("amenityIds", JSON.stringify(amenityIds));
                ...
                {isLoading ? (
                    ...
                    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
                        ...
                        <div className="flex gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-gray-200">
                            ...
                            {/* Submit */}
                            <button type="submit" disabled={isSubmitting} className="flex-1  px-3 py-2 text-sm  sm:px-6 sm:py-3 sm:text-base bg-brand-secondary text-white rounded-lg  hover:bg-brand-secondary/80 font-medium transition-colors  disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1 sm:gap-2 ">
                                {isSubmitting ? (
                                    <>
