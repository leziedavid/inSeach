...
const MySlider: React.FC<SliderProps> = ({ Sliders = [] }) => {
    ...
    useEffect(() => {
        SlidersData.forEach((slider) => {
            ...
            img.onload = () => setLoadedImages(prev => new Set(prev).add(slider.image || '/assets/default-bg.jpg'));
        });
    }, [SlidersData]);
    ...
    useEffect(() => {
        intervalRef.current = setInterval(() => {
            if (!isTransitioning) {
                setCurrentIndex(prev => prev === SlidersData.length - 1 ? 0 : prev + 1);
            }
        }, 3500);
    ...
    const nextSlide = () => handleTransition(() => setCurrentIndex(prev => prev === SlidersData.length - 1 ? 0 : prev + 1));
    const prevSlide = () => handleTransition(() => setCurrentIndex(prev => prev === 0 ? SlidersData.length - 1 : prev - 1));

    return (
        <div className="relative overflow-hidden w-full h-[250px] sm:h-[400px] rounded-xl ">
            ...
            >
                {SlidersData.map((Slider) => {
                    ...
                    return (
                        <div key={Slider.id} className="w-full flex-shrink-0 h-full relative">
                            ...
                            <div
                                className={`absolute inset-0 bg-cover bg-center rounded-xl transition-opacity duration-300 ${isImageLoaded ? 'opacity-100' : 'opacity-0'}`}
                            ...
                            <div className="relative z-10 h-full flex flex-col justify-end p-4 text-white">
                                <h3 className="font-bold text-lg md:text-3xl drop-shadow-md">{Slider.name}</h3>
                                <p className="text-sm md:text-xl opacity-90 -mt-1">{Slider.specialty}</p>

                                <div className="flex items-center mt-1">
                                    <div className="flex">
                                        {Array.from({ length: 5 }, (_, i) => (
                                            <span key={i} className={`text-base md:text-2xl ${i < Slider.rating ? 'text-yellow-400' : 'text-gray-300'}`}>★</span>
                                        ))}
                                    </div>
                                    <span className="text-xs md:text-lg ml-2 opacity-90">{Slider.rating} ({Slider.reviews})</span>
                                </div>

                                {Slider.isToday && (
                                    <button className="mt-3 w-[120px] bg-white/20 hover:bg-white/30 backdrop-blur-md text-white text-xs md:text-base py-1.5 md:py-3 rounded-lg transition">
                                        Book Now
                                    </button>
                                )}
                            </div>
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {SlidersData.map((_, i) => (
                    <button key={i} onClick={() => { if (!isTransitioning) handleTransition(() => setCurrentIndex(i)); }}
                        className={`h-1.5 rounded-full transition-all duration-300 ${i === currentIndex ? 'w-8 bg-white' : 'w-1.5 bg-white/50 hover:bg-white/70'}`}
