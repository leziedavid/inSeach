...
interface JobDetailsAnnonceProps {
    appointment: Appointment;
    parentClose: () => void;
    getUserAppointments: () => void;
}

const JobDetailsAnnonce: React.FC<JobDetailsAnnonceProps> = ({ appointment, parentClose, getUserAppointments }) => {
    ...
    // Fonction pour parser la location
    const parseLocation = (location: any) => {
        ...
        if (typeof location === 'string') {
            try {
                ...
                return { country: location, address: location };
            } catch (error) {
                console.warn('Erreur de parsing JSON pour location:', location, error);
                return { country: location, address: location };
            }
        }
    ...
    return (
        <div className="bg-white p-3">
            ...
            <div className="bg-white rounded-2xl overflow-hidden">
                ...
                {appointment.annonce?.images &&
                    appointment.annonce.images.length > 0 &&
                    isValidImage(appointment.annonce.images[0]) && (
                        <div className="relative w-full h-40 rounded-t-2xl overflow-hidden">
                            <Image
                                src={appointment.annonce.images[0]}
                                alt={appointment.annonce?.title || "Annonce"}
                                fill
                ...
                {(!appointment.annonce?.images ||
                    appointment.annonce.images.length === 0 ||
                    !isValidImage(appointment.annonce.images[0])) && (
                        <div className="relative w-full h-40 rounded-t-2xl overflow-hidden bg-gray-100 flex items-center justify-center">
                ...
                <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                        {/* TITRE */}
                        <h1 className="text-lg font-medium text-gray-900 uppercase">  {appointment.annonce?.title} </h1>
                    ...
                    <div className="flex items-start gap-2 text-gray-700 mb-4 text-sm">
                        ...
                        <div>
                            {appointment.annonce?.gpsLocation && (() => {
                                const loc = parseLocation(appointment.annonce?.gpsLocation);
                                if (!loc) return null;

                                return (
                                    <>
                                        <p className="font-medium">{loc.country || 'Localisation inconnue'}</p>
                                        {loc.city && loc.district && (
                                            <p className="text-sm text-gray-500">  {loc.city}, {loc.district}  </p>
                                        )}
                                        {loc.city && !loc.district && (
                    ...
                    <div className="rounded-xl p-3 mb-5 bg-gray-50 text-sm space-y-2">
                        <div className="flex items-center gap-2 text-gray-700">
                            ...
                            <span>{formattedDate || 'Date non spécifiée'}</span>
                        </div>

                        <div className="flex items-center gap-2 text-gray-700">
                            <Clock className="w-4 h-4 text-gray-400" />
                            <span>{appointment.time || 'Heure non spécifiée'} ({appointment.durationMins || 0} min)</span>
                        </div>
                    </div>
                    ...
                    {activeTab === 'details' && (
                        <div className="space-y-4 text-sm">
                                ...
                                <>
                                    <div className="p-3 bg-yellow-50 rounded-xl text-sm">
                                        ...
                                        <div className="flex items-center gap-1 mb-1 mt-2">
                                            {[...Array(appointment.rating.rating || 0)].map((_, i) => (
                            ...
                            <div className="p-3 bg-gray-50 rounded-xl text-sm">
                                ...
                                <p className="text-gray-600" dangerouslySetInnerHTML={{ __html: appointment.annonce?.description ?? '' }} />
                            </div>
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="bg-gray-50 rounded-xl p-4">
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        <h3 className="text-base font-semibold text-gray-900">
                                            {appointment.client?.name || 'Nom non spécifié'}
                                        </h3>
                                        <p className="text-gray-600 text-sm">{appointment.client?.phone || 'Téléphone non spécifié'}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 text-sm">
                                    ...
                                    <div className="flex items-center gap-2 text-gray-700">
                                        ...
                                        <span>{appointment.client?.email || 'Email non spécifié'}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-blue-50 rounded-xl p-3 text-sm">
                                ...
                                <div className="space-y-1 text-blue-800">
                                    <p>Créé le {appointment.createdAt ? new Date(appointment.createdAt).toLocaleDateString('fr-FR') : 'Date inconnue'}</p>
                                    <p>Mis à jour le {appointment.updatedAt ? new Date(appointment.updatedAt).toLocaleDateString('fr-FR') : 'Date inconnue'}</p>
                                </div>
                            </div>
...
export default JobDetailsAnnonce;