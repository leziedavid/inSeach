...
import { listUserAppointmentAnnonces } from "@/services/appointmentAnnonceService";
import JobDetailsAnnonce from "./JobDetailsAnnonce";

interface RendezVousAnnonceProps {
    type: Historytype;
}

export default function RendezVousAnnonce({ type }: RendezVousAnnonceProps) {
    ...
    const getUserAppointments = async () => {
        try {
            ...
            const items = await listUserAppointmentAnnonces(page);
    ...
    return (
        ...
        <>
            ...
            <div className="flex items-center justify-between mb-8 mt-2">
                <h3 className="font-semibold text-gray-800 text-sm flex items-center gap-2">  <Calendar className="w-4 h-4" />lies des demmandes d'annonce  </h3>
            </div>
                            ...
                            <div key={i} className={`relative flex flex-col border rounded-xl p-3 hover:shadow-md transition  bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 ${isDisabled ? "opacity-50 pointer-events-none" : ""} `} >
                                ...
                                {rdv.scheduledAt && rdv.time && isAppointmentNear(rdv.scheduledAt, rdv.time) && (
                                    <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                                        <button onClick={() => window.open(`tel:${rdv.annonce?.provider?.phone || ""}`)} className="flex items-center gap-1 bg-green-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce hover:scale-105 transition-transform"  >
                                            <Phone className="w-3 h-3" />
                                            Appel
                                        </button>
                                        <button onClick={() => window.open(`https://wa.me/${rdv.annonce?.provider?.phone || ""}`)} className="flex items-center gap-1 bg-[#25D366] text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce hover:scale-105 transition-transform" >
                                ...
                                <div className="absolute top-2 left-2 flex flex-row gap-2 z-10">
                                    <button onClick={() => window.open(`tel:${rdv.annonce?.provider?.phone || ""}`)} className="flex items-center gap-1 bg-blue-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce hover:scale-105 transition-transform" >
                                        <Phone className="w-3 h-3" />
                                        Appel
                                    </button>
                                    <button onClick={() => window.open(`https://wa.me/${rdv.annonce?.provider?.phone || ""}`)} className="flex items-center gap-1 bg-[#25D366] text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg animate-bounce hover:scale-105 transition-transform" >
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-gray-800">{rdv.annonce?.title}</p>
            ...
            {selectedAppointment && (
                <MyModal open={open} onClose={() => setOpen(false)}>
                    <JobDetailsAnnonce  appointment={selectedAppointment}  parentClose={() => setOpen(false)}   getUserAppointments={() => getUserAppointments()} />
                </MyModal>
            )}
                ...
                <MyModal open={openRating} onClose={() => setOpenRating(false)}>
                    <div className="p-4">
                        ...
                        <div className="flex items-center mb-3">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Star key={star} className={`w-6 h-6 cursor-pointer transition-colors  ${star <= (hoverRating || rating) ? "text-yellow-400" : "text-gray-300"} `}  onClick={() => setRating(star)}  onMouseEnter={() => setHoverRating(star)}  onMouseLeave={() => setHoverRating(rating)}  />
                            ))}
                        </div>

                        {/* Commentaire */}
                        <textarea  className="w-full border rounded-lg p-2 text-sm"  placeholder="Votre commentaire..."  rows={4}  value={comment}  onChange={(e) => setComment(e.target.value)} />
            ...
            {completionTarget && (
                <MyModal open={openCompletionForm} onClose={() => setOpenCompletionForm(false)}>
                    <div className="p-4">
                        ...
                        <input  type="number"  className="w-full border rounded-lg p-2 text-sm"  value={completionValue}  onChange={(e) => setCompletionValue(e.target.value)}  placeholder="Entrez un nombre..."  />
